...
#        $AC_TIME_EMI_DUMMY_01[$id]           = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;
        ...
        $AC_TIME_EMI_TRTW[$id]               = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;
        ...
        $AC_TIME_EMI_TRTW_05T[$id]           = &xls_cell_value($Sheet, $_, $platform_scan_idx++) ;
...
# LPDDR3    
	...
	#rank size check END
        ...
        $AC_TIME_VALUE[$id][3] = sprintf("0x%08X",(($AC_TIME_EMI_TWTR[$id]) + ($AC_TIME_EMI_TRRD[$id] << 8) + ($AC_TIME_EMI_TFAW[$id] << 16) + ($AC_TIME_EMI_TRTW[$id] << 24)));
        ...
        $AC_TIME_VALUE[$id][5] = sprintf("0x%08X",(($AC_TIME_EMI_TRAS_05T[$id]) + ($AC_TIME_EMI_TRP_05T[$id] << 2) + ($AC_TIME_EMI_TRPAB_05T[$id] << 4) + 
                                                ...
                                                ($AC_TIME_EMI_TFAW_05T[$id] << 24) + ($AC_TIME_EMI_TRTW_05T[$id] << 26) + ($AC_TIME_EMI_REFCNT_05T[$id] << 28) + 
                                                ($AC_TIME_EMI_REFCNT_FR_CLK_05T[$id] << 30)));
        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $AC_TIME_VALUE[$id][5] . ",\t\t/* U 05 */\n\t\t" ;

        $AC_TIME_VALUE[$id][6] = sprintf("0x%08X",(($AC_TIME_EMI_TXREFCNT_05T[$id]) + ($AC_TIME_EMI_TZQCS_05T[$id] << 2)));
...
#        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . $AC_TIME_VALUE . ",\t\t/* U 12 */\n\t\t" ; 
        ...
        $EMI_SETTINGS[$id] = $EMI_SETTINGS[$id] . "{0,0,0,0,0,0,0,0,0},\t\t/* reserved 9 */\n\t\t" ;
...
#    print FILE pack("a24", "MTK_BLOADER_INFO_v23");
...
sub write_tag_one_element_default_LPDDR3()
{
    ...
    print FILE pack ("L", hex (lc("0xa053a057"))) ;     # EMI_CONA_VAL
    $fs = $fs + 4 ;

    print FILE pack ("L", hex (lc("0x33330000"))) ;     # EMI_CONH_VAL
    $fs = $fs + 4 ;
    ...
    print FILE pack ("L", hex (lc("0x00000000"))) ;     # AC Timing U 05
    $fs = $fs + 4 ;      
    ...
    print FILE pack ("L", hex (lc("0x04210000"))) ;     # EMI_CONF_VAL
    $fs = $fs + 4 ;
    
    print FILE pack ("L", hex ("0")) ;					#reserved[9]  
    print FILE pack ("L", hex ("0")) ;  
    print FILE pack ("L", hex ("0")) ;
    print FILE pack ("L", hex ("0")) ;
    print FILE pack ("L", hex ("0")) ;
