...
my $proj_mak    = $ARGV[0]; # ex: make/pcore/MTXXXX.mak
...
sub recursive_concatenate
{
	...
	while (<$FIN>) {
		...
		if (defined($feature{'MODEM_SPEC'}) and ($feature{'MODEM_SPEC'} ne "NONE") and 
				$line =~ /include\s+(make\/common\/modem_spec\/)/ and 
				($line =~ /MODEM_SPEC/ or $line =~ /$feature{'MODEM_SPEC'}/)) {

			my $modem_spec_mak = "make/common/modem_spec/$feature{'MODEM_SPEC'}.mak";
			...
			&recursive_concatenate($modem_spec_mak, $FOUT);
				...
				$line =~ /include\s+(make\/\$\{CORE_ROOT\}\/custom_config\/)/ and 
				($line =~ /PROJECT_MAKEFILE_EXT/ or $line =~ /$feature{'PROJECT_MAKEFILE_EXT'}/)) {
				
			my $proj_mak_ext = "make/pcore/custom_config/$feature{'PROJECT_MAKEFILE_EXT'}.mak";
