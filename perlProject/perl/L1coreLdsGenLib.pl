...
sub tclGen_verno
{
    return " u0.21_UMOLY + L1coreLdsFrame.pm ".&L1coreLdsFrame::ldsFrame_verno;
...
{
    my ($nTXLength,$nRXLength) = &L1coreSysUtil::DSPTXRX_query_length(GetChip(), undef);
...
sub Default_MemorySetting  # Called by CollectMemorySetting($Setting_href)
{
    ...
    # PCORE_LV
    my $nPCORE_LV_BASE = 0;
    my $nPCORE_LV_LEN = GetPredefinedValue($Setting_href, "PCORE_LV_LEN", undef, L1coreFileInfo::get(L1coreFileInfo::PCORE_LV_SIZE)); # default 6MB


    # ROM
    my $nROM_BASE = $nPCORE_LV_BASE + $nPCORE_LV_LEN;
    ...
    # RAM base
    my $nRAM_BASE = GetPredefinedValue($Setting_href, "RAM_BASE", undef, $nROM_BASE + $nROM_LEN);  
    ...
    SetExistentValueByDefault($Setting_href, "PCORE_LV_LEN", $nPCORE_LV_LEN);
