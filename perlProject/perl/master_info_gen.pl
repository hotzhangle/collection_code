...
# Global Variables
#****************************************************************************
my $g_bBM_NEW = ($BM_NEW eq "bm_new") ? 1 : 0;
my %g_MASTER_INFO;
my %g_MKFILE;
my %g_CmplOpt;
my %g_RelInfo;
...
{
    my $nPCORELVSize = hex(SymFileParser::GetLinkerSymbolAddress("DYNAMIC_CACHEABLE_EXTSRAM_DEFAULT_NONCACHEABLE_RW", LinkerSymPostfix::Base, LinkerSymPrefix::Load))
                     + hex(SymFileParser::GetLinkerSymbolAddress("DYNAMIC_CACHEABLE_EXTSRAM_DEFAULT_NONCACHEABLE_RW", LinkerSymPostfix::Length, LinkerSymPrefix::Image));
    ...
    my $nL1CORELVSize = hex(SymFileParser::GetLinkerSymbolAddress("L1CORE_LV", LinkerSymPostfix::Length, LinkerSymPrefix::Image))
                        + hex(SymFileParser::GetLinkerSymbolAddress("SIG", LinkerSymPostfix::Length, LinkerSymPrefix::Image));
    ...
    return ($nPCORELVSize, $nL1CORELVSize, $nL1CORERWZISize);
}
...
sub get_MASTER_INFO
{
    ...
    my ($nPCORELVSize, $nL1CORELVSize, $nL1CORERWZISize) = &get_MASTER_COMMON_INFO();
    ...
    my $nPcoreDSP_TXLength = SymFileParser::GetLinkerSymbolAddress("PCORE_EXTRAM_TXDATA", LinkerSymPostfix::Length, LinkerSymPrefix::None);
    my $nPcoreDSP_RXLength = SymFileParser::GetLinkerSymbolAddress("PCORE_EXTRAM_RXDATA", LinkerSymPostfix::Length, LinkerSymPrefix::None);
...
sub SetDefaultValue
{
    ...
    my ($nPCORELVSize, $nL1CORELVSize, $nL1CORERWZISize) = (0,0,0);
    
    if(defined $SYM_PATH and -e $SYM_PATH)
    {
        SymFileParser::ParseSYM($SYM_PATH);
    
        ($nPCORELVSize, $nL1CORELVSize, $nL1CORERWZISize) = &get_MASTER_COMMON_INFO();
    }
    ...
    $MASTER_INFO_href->{L1coreFileInfo::PCORE_LV_SIZE} = (($nPCORELVSize==0) ? 0x600000 : $nPCORELVSize); #   6MB
