...
my $project_mk      = $ARGV[3]; # The project makefile path. ex: make/pcore/XXXX.mak
...
#****************************************************************************
# read dsp_project
if (defined($feature{'DSP_PROJECT'}) && $feature{'DSP_PROJECT'} ne "") {
	$dsp_project = $feature{'DSP_PROJECT'};
}
else {
	&error_handler("Fail to get the DSP_PROJECT value from $project_mk!", __FILE__, __LINE__);
}

#read dsp_flavor
if (defined($feature{'DSP_FLAVOR'}) && $feature{'DSP_FLAVOR'} ne "") {
	$dsp_flavor = $feature{'DSP_FLAVOR'};
}
else {
	&error_handler("Fail to get the DSP_FLAVOR value from $project_mk!", __FILE__, __LINE__);
}
...
#my $dsp_bin_file = "${dsp_dir}/${dsp_project}/${dsp_flavor}/${category}/${dsp_project}_${dsp_flavor}_${category}.bin";
my $dsp_bin_file = "${dsp_dir}/$dsp_project/$dsp_flavor/final_bin/dsp.bin";
my ($name, $path, $suffix) = fileparse($dsp_bin_file,qr/\.+?.*/);
my $dsp_dst_file = "$target_dir/DSP_${dsp_version}${suffix}";
my $dsp_dbg_file = glob "${dsp_dir}/$dsp_project/$dsp_flavor/final_bin/DbgInfo_DSP_*";
#if ($dsp_version !~ /DSP/) {
...
my @del_dsp_dbg_files = glob "$target_dir/DbgInfo_DSP_*";
unlink(@del_dsp_dbg_files) if (@del_dsp_dbg_files);
if (-e $dsp_dbg_file){
	copy("$dsp_dbg_file","$target_dir") or &error_handler("copy $dsp_dbg_file to $target_dir $!", __FILE__, __LINE__);	
}
...
Example:
	perl pcore/tools/cp_dsp_files.pl ./pcore/modem/dsp_bin ./build/project/flavor/bin/dsp ./build/project/flavor/bin MTXXXX XX_XX_W1X_1X
