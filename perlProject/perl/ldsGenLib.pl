...
sub ldsGen_verno
{
    return " LR11_v0.02 + ldsFrame.pm ".&ldsFrame::ldsFrame_verno;
...
sub GenSIG_BASE # Fill in RegionList.csv
{
    my ($MEMORYSetting_href, $RegionList_aref, $Index_ref) = @_;
...
sub Default_MemorySetting  # Called by CollectMemorySetting($Setting_href),  fill in MEMORY
{
    ...
    my $nROMBase = 0;
   #$nROMBase = 0x74000000 if( exists $g_MakeFile_ref->{'BASIC_PCORE_IRAM_BOOT'} and $g_MakeFile_ref->{'BASIC_PCORE_IRAM_BOOT'} eq "TRUE" );
    SetExistentValueByDefault($Setting_href, "ROM_BASE", $nROMBase);

    
    
    # ROM, the values could be got by 2-phase linking
    my $nROMSize = FileInfo::get(FileInfo::PCORE_LV_SIZE); # default 14 MB
#    $nROMSize = GetPredefinedValue($Setting_href, "ROM_LEN", "RAM_BASE", 0xE00000); 
#    $nROMSize = CommonUtil::GetKBAligned($nROMSize, sysUtil::GetCopro_KBAlignment("L1CORE")); # default is 4 byte alignment, no need to modify
    SetExistentValueByDefault($Setting_href, "ROM_LEN", $nROMSize);


	# L1CORE_LV, the values could be got by 2-phase linking
    my $nL1CORE_LVBase = $nROMBase + $nROMSize;
    ...
    my $nL1CORE_LVEnd = $nL1CORE_LVBase + FileInfo::get(FileInfo::L1CORE_LV_SIZE);
    ...
    # there is a free space between L1CORE_LV and L1CORE_RQZI


    # L1CORE_RWZI, the values could be got by 2-phase linking
    ...
    # L1CORE TCM, the values could be got by 2-phase linking
