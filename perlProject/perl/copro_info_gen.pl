...
sub get_L1CORE_INFO
{
    ...
    my $nL1coreFirstSectionBase = hex(SymFileParser::GetLinkerSymbolAddress("ROM_GFH", LinkerSymPostfix::Base, LinkerSymPrefix::Image));
    $nL1coreLoadViewSize += &sysUtil::GetSigSize(); # reserve for signature
    ...
    # shared memory size
    my ($region_size_tx, $region_size_rx) = &sysUtil::DSPTXRX_query_length(sysUtil::SwitchToClonedChip(FileInfo::GetChip()), undef);
    ...
    my $nL1coreDSP_TXLength = hex(SymFileParser::GetLinkerSymbolAddress("L1CORE_EXTRAM_TXDATA", LinkerSymPostfix::Length, LinkerSymPrefix::None));
    my $nL1coreDSP_RXLength = hex(SymFileParser::GetLinkerSymbolAddress("L1CORE_EXTRAM_RXDATA", LinkerSymPostfix::Length, LinkerSymPrefix::None));
    ...
    $COPRO_INFO_href->{FileInfo::L1CORE_VECTOR} = hex(SymFileParser::GetLinkerSymbolAddress("INTSRAM_CODE", LinkerSymPostfix::Base, 1));
...
sub SetDefaultValue
{
    ...
    {
        $COPRO_INFO_href->{FileInfo::PCORE_LV_SIZE}    = 0xE00000; # 14MB
        $COPRO_INFO_href->{FileInfo::L1CORE_LV_SIZE}   = 0xE00000; # 14MB
        $COPRO_INFO_href->{FileInfo::L1CORE_RWZI_SIZE} = 0x1400000; # 20MB, it also control l1core RAM size
    }
