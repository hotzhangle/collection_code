...
function mm()
{
    ...
    if [ -f build/core/envsetup.mk -a -f Makefile ]; then
	    ...
	    #add log file by liunianliang begin
	    rm -rf $T/out/target/product/${MTK_PROJECTS}_${FUNCNAME}.log
            ONE_SHOT_MAKEFILE=$M $DRV make -C $T -f build/core/main.mk $MODULES $ARGS  2>&1 | tee -a "$T/out/target/product/${MTK_PROJECTS}_${FUNCNAME}.log"
	    #end
        fi
    fi
}
...
function mma()
{
  ...
  if [ -f build/core/envsetup.mk -a -f Makefile ]; then
    ...
    #add log file by liunianliang begin
    rm -rf $T/out/target/product/${MTK_PROJECTS}_${FUNCNAME}.log
    $DRV make -C $T -f build/core/main.mk $@ all_modules BUILD_MODULES_IN_PATHS="$MY_PWD" 2>&1 | tee -a "$T/out/target/product/${MTK_PROJECTS}_${FUNCNAME}.log"
    #end
  fi
}
    ...
    if [ ! "$JAVA_HOME" ]; then
      case `uname -s` in
          ...
          *)
				...
				export JAVA_HOME=$(gettop)/prebuilts/jdk
...
#add by liunianliang for repo,such as mm,mma
function getHuaqinConfig()
{
    T=$(gettop)
    if [ ! "$T" ]; then
        echo "Couldn't locate the top of the tree.  Try setting TOP." >&2
    ...
    if [ -e "$T/makeMtk.ini" ];then
	export MTK_PROJECTS=$(cat "$T"/makeMtk.ini | grep -w project | awk '{print $3}')
	export HQ_PROJECT=$(cat "$T"/makeMtk.ini | grep -w hq_project | awk '{print $3}')
	export HQ_CLIENT=$(cat "$T"/makeMtk.ini | grep -w client | awk '{print $3}')
	export FLAVOR=""
	export TARGET_PRODUCT=full_$MTK_PROJECTS
	export TARGET_BUILD_TYPE=release
    fi
}
...
function push()
{
    ...
    if [ -e "$T/huaqin/build/shell/ap" ];then
	$T/huaqin/build/shell/ap $T
    fi
}
#end
...
getHuaqinConfig
#add by liunianliang begin
if [ ! -z "AREADLY_SET_ENV" ];then
set_stuff_for_environment
AREADLY_SET_ENV=true
fi
#end
